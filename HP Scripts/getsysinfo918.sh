#!/usr/bin/ksh
# @(#)Revision: 9.18$ $by S. Stechemesser, HP$ $Date: 06/13/2013$
#
export LANG=C
export UNIX95=1
GETSYSINFOVERSION="9.18"
#CKSUM: 754716218
...
NAME=/tmp/sysinfo  #default output name prefix
#add hostname + timestamp by default
NAME=${NAME}_`hostname`_`date +%Y%m%d%H%M` 
...
echo "in case of problems run $0 2>&1 | tee getsysinfo.log "
echo "and send this file also to HP"
echo "options: "
...
echo "-c [all] : run crashinfo on last or all crashdumps (<$MAXBACKDATE days)"
...
if [ $? -eq 1 ] ; then
  ...
  H=`hostname`
  NAME=${NAME}_${H}_`date "+%d.%m.%Y_%H.%M"`
fi
;;
-a)
  MAXFILESIZE=0
  ...
  OSELOGS=1
  RUNCRASHINFO=1
  echo "-a option set :  using options  -h -f -d -net -xp -san -p -sw"
;;
-f)
...
{ #appends a file  to a logfile if it exists
 if [ -f $1 ] ; then
  echo "cat $1 >> $2" ; ( echo "### $1 ###" ; cat $1 ) >> $TDIR/$2 2>&1
 fi
}
ex_log()
{ #executes and logs a command
echo "$1 >> $2" ; ( echo "### $1 ###" ; $1 ) >> $TDIR/$2 2>&1
return $? 
}
ex_log2()
...
{$v=$1;print $_; if($v >= 1.5){exit(1);}}'
 ...
 model | grep -v -q -e "Superdome 2" -e "BL"
 if [ $? -eq 0 -a "X$HPMPLOPT" = "X" ] ; then
 cat<<EOF
...
from the Onboard Administrator (OA).
...
if [ "`uname -r`" = "B.11.31" ] ; then
  ...
  (echo '###ioscan -P health | grep -v -e online -e N/A###'  
  ioscan -P health | grep -v -e online -e N/A )>>$TDIR/ioscan.txt
...
VMGUEST=0
model | grep -q -E "Virtual Machine"
if [ $? -eq 0 ] ; then  #virtual machine guest
 VMGUEST=1
 if [ -x /opt/hpvm/bin/hpvminfo ] ; then
 ex_log /opt/hpvm/bin/hpvminfo hpvmstatus.txt  
 ex_log "/opt/hpvm/bin/hpvminfo -v" hpvmstatus.txt  
 ex_log "/opt/hpvm/bin/hpvminfo -V" hpvmstatus.txt  
 VMHOST=`grep Hostname $TDIR/hpvmstatus.txt | cut -f 2 -d : `
 fi
else  #not a vm guest
if [ -x /opt/hpvm/bin/hpvmstatus ] ; then
   ex_log "/opt/hpvm/bin/hpvmstatus -v"  hpvmstatus.txt
   ex_log "/opt/hpvm/bin/hpvmnet -v"  hpvmstatus.txt
   ex_log "/opt/hpvm/bin/hpvmstatus -V"  hpvmstatus.txt
   ex_log "/opt/hpvm/bin/hpvmstatus -s"  hpvmstatus.txt
   ex_log "/opt/hpvm/bin/hpvmnet -V"  hpvmstatus.txt
...
if [ $secure -eq 0 ] ; then
 if [ -x $NWMGR ] ; then #nwmgr instead of lanscan
  ...
  for I in `$NWMGR |cut -f 1 -d " " | grep lan`;do
   if [ ` echo $I | cut -c 4- ` -ge 900 ] ; then #hp_apa
 ...
 ex_log "netstat -in" lanconfig.txt
 ex_log "netstat -rn" lanconfig.txt
fi
 ...
 if [ $? -ne 0 ] ; then #skip cprop if cimserver not running
  ...
  ex_log2 "/opt/wbem/bin/cimprovider -l -s" diaginfo.txt
   ...
   #capture all except Software related things unless -sw is given
     ...
     'if(/^\s.+\|\s(\S.+)/){$x=$1; if($x!~/Software|Process\s/) 
 ...
 if [ $DUMPSFOUND -ge 1 ] ; then
  ...
  if [ "X$CRASHINFO" != "X" ] ; then
   if [ -x $CRASHINFO ] ; then
    ...
    cd $CRASHDIR
    if [ $RUNCRASHINFO -eq 1 ] ; then #only capture last dump
     CRLIST=`find . -mtime -$MAXBACKDATE -type d -name "crash.*" |\
             cut -c 3-| grep -E "crash.[0-9]+" | tail -1`
    else
     CRLIST=`find . -mtime -$MAXBACKDATE -type d -name "crash.*" |\
             cut -c 3-| grep -E "crash.[0-9]+"`
    fi
    for i in $CRLIST
    do
     if [ -f $i/INDEX ] ; then
...
grep -q -e "EMS hardware monitors are enabled" $TDIR/sfmconfig.txt
...
#finally make a compressed shar archive of the system
jn="n"
...
shar -c -Z -u * > $OUTFILE ;;
1)
tar -cvf - * | gzip -9 -c > $OUTFILE ;;
2) 
tar -cvf - * | gzip -9 -c | uuencode sysinfo.tgz > $OUTFILE ;;
esac
echo cleaning up 
# for security no  rm -r or rm * !!!
for i in config iosca*.txt dmesg.txt swlist.txt syslog.log cstm.info cprop.txt \
...
hpvmstatus.txt lanconfig.txt swagent.log diaginfo.txt sw_inventory.xml mplog* \
